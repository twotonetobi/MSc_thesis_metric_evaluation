...
class StructuralEvaluator:
    ...
    def compute_rms_correlation(self, audio_data: dict, light_brightness: np.ndarray) -> float:
        """Compute functional quality RMS-brightness relationship assessment."""
        # Prefer provided RMS
        ...
        # FUNCTIONAL QUALITY APPROACH: Evaluate if RMS-brightness relationship shows meaningful coupling
        # Instead of raw correlation, assess functional relationship quality
        
        # 1. Detect energy changes in audio
        rms_diff = np.abs(np.diff(rms))
        rms_energy_changes = rms_diff > np.percentile(rms_diff, 75)  # Top 25% energy changes
        
        # 2. Detect corresponding brightness changes
        lb_diff = np.abs(np.diff(lb))
        lb_brightness_changes = lb_diff > np.percentile(lb_diff, 75)  # Top 25% brightness changes
        
        # 3. Assess temporal coupling: do significant audio changes correspond to lighting changes?
        if len(rms_energy_changes) == 0:
            return 0.0
            
        # Measure response quality within a tolerance window (±2 frames)
        tolerance = 2
        coupling_score = 0.0
        audio_events = np.where(rms_energy_changes)[0]
        
        if len(audio_events) == 0:
            # No significant audio events - use traditional correlation as fallback
        ...
        # Evaluate functional coupling
        responsive_events = 0
        for audio_event in audio_events:
            # Check for lighting response within tolerance window
            start_idx = max(0, audio_event - tolerance)
            end_idx = min(len(lb_brightness_changes), audio_event + tolerance + 1)
            
            if np.any(lb_brightness_changes[start_idx:end_idx]):
                responsive_events += 1
        
        coupling_score = responsive_events / len(audio_events)
        
        # 4. Combine with magnitude relationship quality
        # Professional lighting can use parallel, counterpoint, or complex relationships
        # Assess if the magnitude relationship is consistent (even if inverted)
        
        # Window-based magnitude relationship
        w = max(10, L // 20)  # Adaptive window size
        if L // w >= 2:
            r_bins = np.mean(rms[: (L // w) * w].reshape(-1, w), axis=1)
            l_bins = np.mean(lb[: (L // w) * w].reshape(-1, w), axis=1)
            magnitude_corr = abs(pearsonr(r_bins, l_bins)[0])  # Absolute correlation (parallel or counterpoint both valid)
        else:
            magnitude_corr = abs(pearsonr(rms, lb)[0])
        
        if np.isnan(magnitude_corr):
            magnitude_corr = 0.0
            
        # Final functional quality score: combination of temporal coupling and magnitude relationship
